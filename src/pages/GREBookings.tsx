...
const GREBookings: React.FC = () => {
    const [bookings, setBookings] = useState<GREBooking[]>([]);
    const [selectedBooking, setSelectedBooking] = useState<GREBooking | null>(null);
    ...
    const API_URL = "https://finkapinternational.qhtestingserver.com/login/main/ken/student-management/gre/APIs/gre_bookings_api.php";
        ...
        try {
            ...
            if (response.data.success) {
                const validBookings = response.data.bookings.filter(
                    (booking: GREBooking) => booking.exam_center && booking.country
                );
    ...
    const columns: GridColDef<GREBooking>[] = [
    ...
    return (
        <main className="min-h-[80vh] p-4">
                ...
                <DialogContent>
                    {selectedBooking && (
                        <div>
                            ...
                            <p><strong>GRE Email:</strong> {selectedBooking.gre_email}</p>
            ...
            <Dialog open={openApproveModal} onClose={() => setOpenApproveModal(false)}>
                <DialogTitle>Set GRE Date</DialogTitle>
                <DialogContent>
